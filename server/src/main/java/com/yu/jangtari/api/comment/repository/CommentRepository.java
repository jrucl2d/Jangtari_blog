...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query("select c from Comment c where c.id = :commentId and c.member.id = :memberId and c.deleteFlag.isDeleted = false")
    Optional<Comment> findByIdAndMemberId(@Param(value = "commentId") Long commentId, @Param(value = "memberId") Long memberId);
}
